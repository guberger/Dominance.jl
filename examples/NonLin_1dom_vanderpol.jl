...
include("./_vanderpol_.jl")

sleep(0.1) # used for good printing
println("Plot NonLin 1-dom vanderpol")
...
nsub = (3, 3)
DDF_norm_inf(x) = -DO.tensor3d_normInf2matp(DDVanderpol(x), Inf)
DDF_norm_2(x) = -DO.tensor3d_normInf2matp(DDVanderpol(x), 2)
f_opt, ~ = DO.minimize_over_domain(DDF_norm_inf, domain, nsub)
println(-f_opt)
bound_DDF_inf = -f_opt
# bound_DDF_inf = opnorm(U, Inf)*3*sqrt(3)/8

sys = DO.ContSystemRK4(tstep, Vanderpol, DVanderpol, 0.1, 0.0*bound_DDF_inf, 4)
