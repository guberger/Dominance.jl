...
#=
Just replaced the coefficient of sin(x[1]) (compared to NonLin_2dom_duffing) by
0.15, instead of 0.3, (also in duffing_deriv) in order to have that the system
can be shown to be p-dominant with the trivial automaton, obtained by changing
the edges in (i, i, j, [1, 2], [1, 1]) instead of (i, j, i, [1, 2], [1, 1]) in
edge_list.
=#
...
function duffing_map!(x, y)
    y[1] = x[1] + 0.3*x[2]
    y[2] = -0.15*x[1] + 0.15*sin(x[1]) + 0.7*x[2] + 0.03*x[3]
    y[3] = -1.5*x[1] + 0.925*x[3]
end

function duffing_deriv(cosx)
    return [1.0 0.3 0.0;
        (-0.15 + 0.15*cosx) 0.7 0.03;
        -1.5 0.0 0.925]
end
